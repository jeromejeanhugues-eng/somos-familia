  ...
  async function onSubmit(data: FormData) {
    ...
    try {
      const res = await fetch('https://formspree.io/f/mrejlggn', {
        ...
        body: JSON.stringify({ ...data, _subject: 'Nouvelle candidature - Bénévolat' }),
      });
      if (res.ok) {
        setSubmitted(true);
      } else {
  ...
  if (submitted) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-light text-lg font-playfair">Merci ! Votre candidature a bien été reçue. Nous vous contacterons très bientôt.</p>
      </div>
    );
  }
